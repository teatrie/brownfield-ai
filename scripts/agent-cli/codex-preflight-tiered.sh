#!/usr/bin/env bash
# Codex pre-flight: pick local vs container path. Authentication is
# NOT verified here — assumed valid if the local CLI is on PATH or, in
# the container path, if OPENAI_API_KEY is set. Auth failures surface
# at execution time and are reported to the user (interactive) or
# logged (headless).
#
# Modes:
#   local      — `codex` is on PATH; invoke against the local CLI
#                (OAuth or API key, indistinguishable from preflight).
#   container  — no local `codex`; OPENAI_API_KEY is set; invoke via
...
# (default `tmp/.codex-preflight-cache.json`) and reused for up to
...
CACHE_FILE="${PREFLIGHT_CACHE_FILE:-tmp/.codex-preflight-cache.json}"
...
if command -v codex >/dev/null 2>&1; then
  result='{"mode":"local","codex":{"cli":true}}'
elif [ -n "${OPENAI_API_KEY:-}" ]; then
  result='{"mode":"container","codex":{"cli":false,"api_key":true}}'
else
  ...
  printf '{"mode":"none","codex":{"cli":false,"api_key":false}}\n'
  exit 1
fi
