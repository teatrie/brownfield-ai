...
if [ "$TARGET" == "scripts" ]; then
    CHANGED_SCRIPTS=$(echo "$CHANGED_FILES" | grep -E "^(scripts/|tests/scripts/|ci/|tests/ci/|\.claude/hooks/|tests/hooks/|\.claude/agents/|tests/agents/|docker/shared/|docker/agent-cli/)" || true)
