...
- **Synchronous Dispatch for No-`SendMessage` Agent Types (Result-Relay Guard — Principle 18 Enforcement)**: A **named** background in-process `Agent` dispatch can return its result to the caller only via `SendMessage` (an *unnamed* background subagent instead returns its final message via a completion task-notification — see the reserved cases at the end of this bullet). **No agent definition under [.claude/agents/](../.claude/agents) declares `SendMessage`** — `code-review*`, `codex-reviewer*`, `gemini-reviewer*`, `copilot-reviewer*`, `explore*`, `qa-standards*`, `qa-lint`, `qa-test`, `task`, `deep-researcher*`, `planner*`, `orchestrator`, `general-purpose*`, and `tdd-*` are all Read-restricted or Read/Edit/Bash-restricted. Consequently, a **named** background dispatch of any of them is promoted to a persistent in-process teammate that completes its analysis, emits the deliverable as its final-turn text, then goes idle — with no tool to push that text back. The deliverable is stranded in the subagent transcript; reading that transcript file directly overflows the caller's context (it is the full JSONL conversation, not a summary), so the only recovery is to re-run the whole dispatch — pure token waste, and an easy mistake to make with a review panel. **RULE: never pass `name` when dispatching a no-`SendMessage` agent type; where the `Agent` tool also exposes `run_in_background`, set it to `false`.** `name` is the primary lever, and the portable one: upstream states this rule as "set `run_in_background: false`", but **some `Agent` tool surfaces expose no `run_in_background` parameter at all** — on those the upstream phrasing is unactionable and the rule silently fails to bind, whereas `name` is always present and is what actually promotes a dispatch to a persistent mailbox-routed teammate. An unnamed subagent returns its final message via a completion task-notification and cannot strand. Concurrency is unaffected either way: multiple `Agent` calls issued in a single message run in parallel and all return together, so a review panel loses no wall-clock. Named dispatch is reserved for `agent-team` tmux teammates, which are full `claude` processes carrying `SendMessage`/`TaskUpdate`. Decision test before naming any `Agent` call: *does this `subagent_type` grant `SendMessage`?* If no → do not name it.
- **Recovering a Stranded Bridge Review**: when a `codex-reviewer*` / `gemini-reviewer*` / `copilot-reviewer*` dispatch strands, the underlying CLI has usually *already run to completion* — the wrapper writes `tmp/<bridge>-review-output-<ROUND>.md` regardless of whether the subagent relays. Check `ls -lat tmp/` before concluding the review failed, and compare timestamps against `tmp/<bridge>-exit.json`: a first attempt can record an error signal while its retry succeeded, leaving the exit file *older* than the output file. Re-running a completed review is the waste this guard rail exists to prevent.
