...
def test_all_available(tmp_path: Path) -> None:
    ...
    env["COPILOT_GITHUB_TOKEN"] = "mock-token"
    env["GEMINI_API_KEY"] = "mock-key"

    script = Path("scripts/agent-cli/preflight.sh").absolute()
...
PREFLIGHT_SCRIPT_PATH = Path("scripts/agent-cli/gemini-preflight-tiered.sh")
...
def _run_preflight(
    *,
    ...
    """Run the gemini preflight script with a controlled environment.
...
class TestLocalCliAvailable:
    ...
    def test_returns_local_mode(self, tmp_path: Path) -> None:
        ...
        result = _run_preflight(mock_gemini_bin=bin_dir)
        assert result.returncode == 0
...
class TestUnavailable:
    ...
    def test_returns_none_mode_and_exit_1(self, tmp_path: Path) -> None:
        bin_dir = _isolated_bin_without_cli(tmp_path, cli_names=("gemini",))
        result = _run_preflight(env_override={"PATH": str(bin_dir)})
        assert result.returncode == 1
        data: dict[str, Any] = json.loads(result.stdout.strip())
...
REVIEW_SCRIPT_PATH = Path("scripts/agent-cli/gemini-review.sh").resolve()
# Canonical reviewer templates — the wrapper resolves
# ``.claude/prompts/reviewer/<REVIEW_TYPE>.md`` relative to CWD, so fake
# workspaces symlink this directory into place.
REAL_TEMPLATES_DIR = Path(__file__).resolve().parents[2] / ".claude" / "prompts" / "reviewer"
...
def _run_review_script(
    *,
    ...
    """Run gemini-review.sh with controlled environment.

    Args:
        env_override: Extra env vars to merge with minimal base.
        cwd: If set, the working directory for the subprocess.
...
class TestTokenGuardHostContext:
    ...
    def test_host_context_no_api_key_does_not_emit_unavailable(self) -> None:
    ...
    def test_no_context_no_api_key_emits_unavailable(self, tmp_path: Path) -> None:
        ...
        Under the TODO-0092 Phase A contract the wrapper validates
        ``REVIEW_TYPE``/``DIFF_FILE`` before the token guard, so the
        fake workspace must provide both (plus the reviewer templates).
        """
        (tmp_path / "tmp").mkdir()
        (tmp_path / "tmp" / "qa-diff.txt").write_text("diff --git a/x b/x\n+a\n")
        _link_reviewer_templates(tmp_path)
        _run_review_script(
            env_override={
                "REVIEW_TYPE": "diff",
        ...
        exit_json = tmp_path / "tmp" / "gemini-exit.json"
...
def _setup_review_workspace(tmp_path: Path) -> None:
    """Create the minimal file structure required by gemini-review.sh.

    Under the TODO-0092 Phase A contract this means:
...
class TestFallbackSignal:
    ...
    def test_slow_failure_exits_3_with_fallback_signal(self, tmp_path: Path) -> None:
        ...
        _setup_review_workspace(tmp_path)
    ...
    def test_success_with_timeout_exits_0(self, tmp_path: Path) -> None:
        ...
        output = tmp_path / "tmp" / "gemini-review-output-1.md"
