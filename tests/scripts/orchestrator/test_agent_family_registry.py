...
``agent_family`` enum and the ``ENVELOPE_AGENTS`` regex in
``ci/lint_staged.sh`` — cannot import Python at runtime (JSON has no
import; bash CI must stay portable across macOS / Linux without a
Python bootstrap). These tests cross-check those two surfaces against
the registry so drift surfaces in CI rather than silently in
production.
...
def test_lint_staged_regex_covers_every_registry_agent_id() -> None:
    """[TODO-0146] Shell ENVELOPE_AGENTS regex MUST match every registry agent ID.

    The shell regex is hand-maintained (CI bash can't import Python) but
    MUST cover every realized agent ID in the registry. A registry agent
    ID that the shell regex fails to match would silently bypass the
    lint-staged fast-path on commits that touch only that agent.
    """
    shell_text = _LINT_STAGED_PATH.read_text(encoding="utf-8")
    match = re.search(r"^ENVELOPE_AGENTS=.*?-E\s+'([^']+)'", shell_text, re.MULTILINE)
    assert match, (
        f"Could not locate ENVELOPE_AGENTS regex in {_LINT_STAGED_PATH}. "
        f"The test depends on the line shape `ENVELOPE_AGENTS=$(... -E '<regex>' ...)`."
    )
    shell_regex = re.compile(match.group(1))
    registry_agent_ids = lint_staged_agent_ids()
    unmatched = [agent_id for agent_id in registry_agent_ids if not shell_regex.match(f".claude/agents/{agent_id}.md")]
    assert not unmatched, (
        f"ci/lint_staged.sh ENVELOPE_AGENTS regex does not match registry agent IDs: {unmatched}. "
        f"Widen the regex in ci/lint_staged.sh or remove the agent ID from the registry."
    )
...
def test_lint_staged_regex_forward_compat_slack_pins_known_lookalikes() -> None:
    ...
    The pair (``test_lint_staged_regex_covers_every_registry_agent_id`` +
    ...
    coverage test ``test_lint_staged_regex_covers_every_registry_agent_id``
    ...
    assert match, "ENVELOPE_AGENTS regex not found"
    shell_regex = re.compile(match.group(1))
    registry_agent_ids = set(lint_staged_agent_ids())
...
def test_registry_agent_files_exist() -> None:
    ...
    missing: list[str] = []
    for agent_id in lint_staged_agent_ids():
